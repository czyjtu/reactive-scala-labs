...
object PaymentService {
  ...
  def apply(
    ...
    Behaviors.setup { context =>
      sendRequest(method, context)
      expectResponse(payment)
    }

  def expectResponse(payment: ActorRef[Response]): Behavior[HttpResponse] =
    Behaviors.receiveMessage {
      case HttpResponse(StatusCodes.OK, _, _, _) =>
        payment ! PaymentSucceeded
        Behaviors.stopped
      case HttpResponse(StatusCodes.InternalServerError, _, _, _) |
           HttpResponse(StatusCodes.RequestTimeout, _, _, _) |
           HttpResponse(StatusCodes.ImATeapot, _, _, _) =>
        throw PaymentServerError()
      case HttpResponse(StatusCodes.NotFound, _, _, _) =>
        throw PaymentClientError()
    }
  def sendRequest(method: String, context: ActorContext[HttpResponse]): Unit = {
